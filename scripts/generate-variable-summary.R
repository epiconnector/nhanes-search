...
## WARNING: Even when using the database, this script will take a
## while to run as it will load and process each table.

if (basename(getwd()) != "docs") stop("This script should be run in the nhanes-search/docs/ folder")
...
tableDesc <- metadata_tab(UseConstraints = TRUE)
## optional: drop pre-pandemic
## tableDesc <- subset(tableDesc, !startsWith(TableName, "P_"))
dim(tableDesc)
